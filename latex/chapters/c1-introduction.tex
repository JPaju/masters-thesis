...
The thesis studies three different approaches to side effects; unrestricted side effects, monads, and algebraic effects and handlers, \new{which are studied with Koka and Unison programming languages. The thesis also provides a brief overview of a bleeding-edge approach to managing effect called capability-based effects.} As a vehicle for concrete experimentation, we use and study a Scala library called ZIO, which can support all of the above programming approaches. The approaches are studied in terms of how they affect the implementation of programs. The thesis seeks answers to the following research questions about different approaches to managing effects:
\begin{description}
...
\new{
These are broad questions and it's worth mentioning that a reader who is looking for a unambiguous, concise and one-size-fits-all answer to these questions might be disappointed by the subject's multifaceted nature. Instead, it is essential to gain a comprehensive understanding on the domain of computational effects in order to analyze the approaches from different perspectives.
}
