...
\todo{Mainitse että alg effseissä efektit kulkevat expressioneista ulospäin, capability effecteissä taas ulkoa expressioneitä "kohti"}

\new{
The proposals of modeling effects with capabilities are promising. It should be possible to define effect polymorphic higher-order functions without the need to mention any effect variables in the type signature. For example, an effectful \inlinecode{List.map} function in Scala would have signature \inlinescala{def map(f : A => B): List [B]}, which is exactly the same as the current \inlinecode{map} in Scala. This is possible because \inlinecode{map} does not require any \emph{additional} capabilities in addition to capabilities required by \inlinecode{f}. If \inlinecode{f} requires any capabilities, the context in which \inlinecode{map} is called, must have/provide those capabilities. ~\cite{scoped-capabilities}
}
